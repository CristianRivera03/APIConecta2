...
namespace Conecta2.API.Controllers
{
    ...
    public class RoleController : ControllerBase
    {
        ...
        {
            var rsp = new Response<RoleDTO>();
            try
            {
                rsp.status = true;
                rsp.value = await _roleService.CreateRole(request);
                rsp.msg = "Rol creado exitosamente";
                return Ok(rsp);
            }
            catch (Exception)
            {
                rsp.status = false;
                rsp.msg = "Ocurrió un error al crear el rol";
        ...
        public async Task<IActionResult> UpdatePermissions([FromBody] UpdatePermissionsDTO request)
        {
